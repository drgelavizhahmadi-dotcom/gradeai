...
async function main() {
  ...
  try {
    ...
    try {
      ...
      console.log(`  - Grade severity: ${aiAnalysis.gradeInterpretation.severity}`)
      console.log(`  - Concern level: ${aiAnalysis.gradeInterpretation.concernLevel}/10`)
      console.log(`  - Strengths: ${aiAnalysis.strengths.length}`)
      console.log(`  - Weaknesses: ${aiAnalysis.weaknesses.length}`)
      console.log(`  - Resources (free): ${aiAnalysis.resources.free.length}`)
      console.log(`  - Resources (paid): ${aiAnalysis.resources.paid.length}`)
      console.log(`  - Prediction: ${aiAnalysis.prediction.endOfSemester}`)
    } catch (error) {
