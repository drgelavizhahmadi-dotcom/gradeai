...
export async function analyzeWithClaudeVision(
  ...
  console.log('[Claude Vision] Starting SINGLE-LAYER analysis');
  ...
  try {
    ...
    // Single-layer: send images + prompt, get structured JSON directly
    content.push({
      type: 'text',
    ...
    console.log(`[Claude Vision] Tokens: input=${response.usage.input_tokens}, output=${response.usage.output_tokens}`);
    ...
    console.log('[Claude Vision] ✓ Teacher comment:', typeof parsed.teacherFeedback?.mainComment === 'string'
      ? parsed.teacherFeedback.mainComment.substring(0, 50)
      : parsed.teacherFeedback?.mainComment?.text?.substring(0, 50) || 'NOT FOUND');
