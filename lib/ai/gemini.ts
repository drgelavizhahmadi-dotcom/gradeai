...
export async function analyzeTest(params: AnalyzeTestParams): Promise<TestAnalysis> {
  ...
  try {
    ...
    console.log('[Gemini AI]   Grade interpretation:', analysis.gradeInterpretation.meaning.substring(0, 100) + '...')
    console.log('[Gemini AI]   Strengths:', analysis.strengths.length)
    console.log('[Gemini AI]   Weaknesses:', analysis.weaknesses.length)
    console.log('[Gemini AI]   Action plan priorities:', Object.keys(analysis.actionPlan).length)
